        ...
        <article className="dashboard-card xp-card">
          <div className="panel-heading">
            <div>
              ...
              <p>{gamification.xp} XP acumulados</p>
            </div>
          ...
          <p>Próximo nível em {gamification.next_level_xp} XP.</p>
        </article>
          ...
          {expensesByCategory.length === 0 ? (
            ...
            <div className="dashboard-bar-list">
              {expensesByCategory.map((item) => (
                <article className="dashboard-bar-row" key={item.category}>
                  <div>
            ...
            <div className="monthly-chart">
              {monthlyEvolution.map((item) => (
                <article className="monthly-chart-row" key={item.month}>
                  ...
                  <div>
                    <span className="income-bar" style={{ width: `${Math.round((Number(item.income) / maxMonthlyValue) * 100)}%` }} />
                    <span className="expense-bar" style={{ width: `${Math.round((Number(item.expenses) / maxMonthlyValue) * 100)}%` }} />
                  </div>
                  <small>R {formatCurrency(item.income)} · D {formatCurrency(item.expenses)}</small>
                </article>
              ))}
            </div>
          )}
